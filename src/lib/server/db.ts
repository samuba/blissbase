...
if (process.env.E2E_TEST === 'true') {
	// Dynamic import for PGlite to avoid loading it in production
	const { PGlite } = require('@electric-sql/pglite');
	const { drizzle: pgliteDrizzle } = require('drizzle-orm/pglite');
	const { cube } = require('@electric-sql/pglite/contrib/cube');
	const { earthdistance } = require('@electric-sql/pglite/contrib/earthdistance');
	...
	// Initialize extensions and schema asynchronously
	const initDb = async () => {
	...
	// Store init promise so we can await it if needed
	(db as any).$initPromise = initDb();
} else {

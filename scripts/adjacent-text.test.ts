...
describe('Adjacent Text Message Processing', () => {
    ...
    it('should handle text messages with adjacent text details', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: combinedText, messageDate: date, timezone: `germany`, imageInputs: [], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: combinedText, messageDate: date, timezone: `germany`, imageInputs: [], eventIsDefinitelyConscious: false }));
    ...
    it('should handle image messages with adjacent text details', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: combinedText, messageDate: date, timezone: `germany`, imageInputs: [imageUrl], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: combinedText, messageDate: date, timezone: `germany`, imageInputs: [imageUrl], eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle messages with no adjacent text', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: mainText, messageDate: date, timezone: `germany`, imageInputs: [], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: mainText, messageDate: date, timezone: `germany`, imageInputs: [], eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle mixed content with both text and images', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: combinedText, messageDate: date, timezone: `germany`, imageInputs: imageUrls, eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: combinedText, messageDate: date, timezone: `germany`, imageInputs: imageUrls, eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(true);

...
describe('Image Processing for Event Extraction', () => {
    ...
    it('should handle image-only messages with event data', async () => {
        ...
        const result = await aiExtractEventData({ message: ``, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/event-flyer.jpg`], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: ``, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/event-flyer.jpg`], eventIsDefinitelyConscious: false }));
    ...
    it('should handle image-only messages without event data', async () => {
        ...
        // Test the AI function with an image URL
        const date = new Date();
        const result = await aiExtractEventData({ message: ``, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/random-image.jpg`], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: ``, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/random-image.jpg`], eventIsDefinitelyConscious: false }));
    ...
    it('should handle mixed text and image messages', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: `Join our yoga retreat! See flyer for details.`, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/yoga-flyer.jpg`], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: `Join our yoga retreat! See flyer for details.`, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/yoga-flyer.jpg`], eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle text messages with adjacent images', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: `Join our art workshop!`, messageDate: date, timezone: `germany`, imageInputs: adjacentImages, eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: `Join our art workshop!`, messageDate: date, timezone: `germany`, imageInputs: adjacentImages, eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle multiple images in a message', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: ``, messageDate: date, timezone: `germany`, imageInputs: imageUrls, eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: ``, messageDate: date, timezone: `germany`, imageInputs: imageUrls, eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle existing source detection in images', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData({ message: ``, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/sei.jetzt-event.jpg`], eventIsDefinitelyConscious: false });

        expect(aiExtractEventData).toHaveBeenCalledWith(expect.objectContaining({ message: ``, messageDate: date, timezone: `germany`, imageInputs: [`https://example.com/sei.jetzt-event.jpg`], eventIsDefinitelyConscious: false }));
        expect(result.hasEventData).toBe(false);

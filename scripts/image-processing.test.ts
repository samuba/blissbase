...
describe('Image Processing for Event Extraction', () => {
    ...
    it('should handle image-only messages with event data', async () => {
        ...
        const result = await aiExtractEventData('', date, 'germany', ['https://example.com/event-flyer.jpg']);

        expect(aiExtractEventData).toHaveBeenCalledWith('', date, 'germany', ['https://example.com/event-flyer.jpg']);
    ...
    it('should handle image-only messages without event data', async () => {
        ...
        // Test the AI function with an image URL
        const date = new Date();
        const result = await aiExtractEventData('', date, 'germany', ['https://example.com/random-image.jpg']);

        expect(aiExtractEventData).toHaveBeenCalledWith('', date, 'germany', ['https://example.com/random-image.jpg']);
    ...
    it('should handle mixed text and image messages', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData('Join our yoga retreat! See flyer for details.', date, 'germany', ['https://example.com/yoga-flyer.jpg']);

        expect(aiExtractEventData).toHaveBeenCalledWith('Join our yoga retreat! See flyer for details.', date, 'germany', ['https://example.com/yoga-flyer.jpg']);
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle text messages with adjacent images', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData('Join our art workshop!', date, 'germany', adjacentImages);

        expect(aiExtractEventData).toHaveBeenCalledWith('Join our art workshop!', date, 'germany', adjacentImages);
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle multiple images in a message', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData('', date, 'germany', imageUrls);

        expect(aiExtractEventData).toHaveBeenCalledWith('', date, 'germany', imageUrls);
        expect(result.hasEventData).toBe(true);
    ...
    it('should handle existing source detection in images', async () => {
        ...
        const date = new Date();
        const result = await aiExtractEventData('', date, 'germany', ['https://example.com/awara-event.jpg']);

        expect(aiExtractEventData).toHaveBeenCalledWith('', date, 'germany', ['https://example.com/awara-event.jpg']);
        expect(result.hasEventData).toBe(false);
